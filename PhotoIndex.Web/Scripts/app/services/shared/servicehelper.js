app.factory('serviceHelperSvc', ['$http', '$resource', function ($http, $resource) {
    ...
    return {
        ...
        Photo: $resource(buildUrl('api/Photos/:photoId'),
            { photoId: '@Id' },
            {
                'update': { method: 'PUT' },
                getPagedItems: { url: buildUrl("api/Photos?count=:count&page=:page"), method: 'GET', params: { count: '@count', page: '@page' } }
            }),
        ...
        Location: $resource(buildUrl('api/Locations/:locationId'),
            { locationId: '@Id' },
            {
                'update': { method: 'PUT' }
            }),
